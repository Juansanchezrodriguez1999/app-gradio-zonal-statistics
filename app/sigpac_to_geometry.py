...
def _convert_sigpac_to_cadastral_reference(province: int, municipality: int, polygon: int, parcel: int, precinct: int, section: str = "A") -> str:
    """
...
def _generate_cadastral_reference(reference: str) -> str:
    ...
    for i in range(7):
        sum_primary_digits += _calculate_character_value(separated_ref[i], position_weights[i])
        sum_secondary_digits += _calculate_character_value(separated_ref[i + 7], position_weights[i])
...
def sigpac_to_geometry(province: int, municipality: int, polygon: int, parcel: int, precinct: int):
    """
